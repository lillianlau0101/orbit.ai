...
## 4. 增量功能需求
- FR-V2-FB-1：云函数 `getMonthDays` 返回 `{year, month, wrote[{day, mood, color}]}`
- FR-V2-FB-2：云函数 `getDiaryList` 入参 `{year, month, day}` 返回多篇日记数组
- FR-V2-FB-3：云函数 `saveDiary` 入参 `{year, month, day, text, mood, color}`，幂等写入
- FR-V2-FB-4：云函数 `updateDiary` 入参 `{_id, text, mood, color}`，允许修改
- FR-V2-FB-5：云函数 `deleteDiary` 软删除，字段 `deleted: true`
- FR-V2-FB-6：月视图无限滑动，左右切换月份无闪烁
- FR-V2-FB-7：设置页配色选择器，5 组预设，实时生效
...
3. 按 US-V2-FB-001 → US-V2-FB-004 顺序实现 → 推 `main` → 截图验收
