. 增量概述
在现有 AI 聊天底子上增加「日历导航 + 日记聚合 + 月视图」，允许同一天写多篇日记，每篇可选情绪色。月视图用情绪圆点展示当日状态。不破坏原有 chat 结构，仅新增 diary 集合与日历 UI。
2. 增量目标
14 天内发版，零外部成本
...
Next：
新建 diary 集合 + 云函数 getMonthDays/getDiaryList/saveDiary/updateDiary/deleteDiary
新建页面 pages/calendar/calendar + pages/diary/diary + pages/settings/color-picker
按 US-V2-001 → US-V2-004 顺序实现 → 推 main → 截图验收
